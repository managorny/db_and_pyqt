...
def make_sock_send_msg_get_answer():
    ...
    if not client_account_name or not client_account_password:
        ...
        if start_dialog.ok_pressed:
            ...
            logger.debug(f'USERNAME = {client_account_name}, PASSWORD = {client_account_password}.')
        else:
            exit(0)
    ...
    try:
        sock = ClientInteraction(server_address, server_port,
                                 client_account_name, database, client_account_password, keys)
        ...
        message = QMessageBox()
        message.critical(start_dialog, f'Ошибка сервера: {ex}')
        logger.critical(
